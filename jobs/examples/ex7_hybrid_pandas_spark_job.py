...
class Job(ETLBase):
    OUTPUT_TYPES = {"session_id": types.VARCHAR(100), "count_events": types.INT()}
    ...
    def transform(self, some_events, other_events):
        ...
        df1 = some_events_pd[
            some_events_pd.apply(
                lambda row: row["action"] == "searchResultPage"
                and float(row["n_results"]) > 0,
                axis=1,
            )
        ]
        df2 = pd.merge(
            left=df1,
            right=other_events_pd,
            how="inner",
            left_on="session_id",
            right_on="session_id",
            indicator=True,

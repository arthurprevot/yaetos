...
LOCAL_APP_FOLDER = os.environ.get(
    "PYSPARK_AWS_ETL_HOME", ""
)  # PYSPARK_AWS_ETL_HOME set to end with '/', TODO: rename env var to YAETOS_HOME, and check if LOCAL_APP_FOLDER and LOCAL_JOB_REPO_FOLDER are used properly in code. Saw strange cases.
...
]  # necessary for reading/writing to redshift, mysql & clickhouse using spark connector.
PACKAGES_EMR_ALT = [
    "io.github.spark-redshift-community:spark-redshift_2.12:5.0.3",
    "org.apache.spark:spark-avro_2.12:3.1.1",
    "mysql:mysql-connector-java:8.0.22",
    "org.postgresql:postgresql:42.2.18",
]  # same but compatible with spark 3.
PACKAGES_LOCAL = PACKAGES_EMR + [
    "com.amazonaws:aws-java-sdk-pom:1.11.760",
    "org.apache.hadoop:hadoop-aws:2.7.0",
]
PACKAGES_LOCAL_ALT = PACKAGES_EMR_ALT + [
    "com.amazonaws:aws-java-sdk-pom:1.11.760",
    "org.apache.hadoop:hadoop-aws:2.7.0",
]  # will probably need to be moved to hadoop-aws:3.2.1 to work locally.
...
class ETLBase:
    ...
    def __init__(self, pre_jargs={}, jargs=None, loaded_inputs={}):
        ...
        if self.jargs.manage_git_info:
            ...
            [
                git_yml.pop(key, None)
                for key in ("diffs_current", "diffs_yaetos")
                if git_yml
            ]
    ...
    def etl_multi_pass(self, sc, sc_sql, loaded_inputs={}):
        ...
        while (
            needs_run
        ):  # TODO: check to rewrite as for loop. Simpler and avoiding potential infinite loops.
            ...
            if self.jargs.merged_args.get("job_increment") == "daily":
                if ii == 1:
                    ...
                    periods = Period_Builder().get_last_output_to_last_day(
                        last_run_period, first_day
                    )

                if len(periods) == 0:
                    logger.info(
                        "Output up to date. Nothing to run. last processed period={} and last period from now={}".format(
                            last_run_period, Period_Builder.get_last_day()
                        )
                    )
                    output = su.create_empty_sdf(sc_sql)
                    self.final_inc = (
                        True  # remove "self." when sandbox job doesn't depend on it.
                    )
                else:
            ...
            if (
                self.jargs.rerun_criteria == "last_date"
            ):  # i.e. stop when reached final increment, i.e. current period is last to process. Pb: can go in infinite loop if missing data.
                needs_run = not self.final_inc
            elif (
                self.jargs.rerun_criteria == "output_empty"
            ):  # i.e. stop when current inc is empty. Good to deal with late arriving data, but will be a pb if some increment doesn't have data and will never have.
    ...
    def etl_one_pass(self, sc, sc_sql, loaded_inputs={}):
        ...
        logger.info(
            "-------Starting running job '{}'--------".format(self.jargs.job_name)
        )
        start_time = time()
        self.start_dt = (
            datetime.utcnow()
        )  # attached to self so available within "transform()" func.
        output, schemas = self.etl_no_io(sc, sc_sql, loaded_inputs)
        if output is None:
            if self.jargs.is_incremental:
                logger.info(
                    "-------End job '{}', increment with empty output--------".format(
                        self.jargs.job_name
                    )
                )
                ...
                logger.info(
                    "-------End job '{}', no output--------".format(self.jargs.job_name)
                )
            # TODO: add process time in that case.
            return None

        if not self.jargs.no_fw_cache or (
            self.jargs.is_incremental and self.jargs.rerun_criteria == "output_empty"
        ):
            ...
            try:
                ...
                logger.info(
                    "Warning: Failed showing table sample with error '{}'.".format(e)
                )
                pass
            count = output.count()
            ...
            if self.jargs.output.get("df_type", "spark") == "spark":
                logger.info(
                    "Output data types: {}".format(
                        pformat([(fd.name, fd.dataType) for fd in output.schema.fields])
                    )
                )
        ...
        logger.info(
            "Process time to complete (post save to file but pre copy to db if any, also may not include processing if output not saved): {} s".format(
                elapsed
            )
        )
        ...
        if (
            self.jargs.merged_args.get("copy_to_redshift")
            and self.jargs.enable_redshift_push
        ):
            self.copy_to_redshift_using_spark(
                output
            )  # to use pandas: self.copy_to_redshift_using_pandas(output, self.OUTPUT_TYPES)
        if (
            self.jargs.merged_args.get("copy_to_clickhouse")
            and self.jargs.enable_redshift_push
        ):  # TODO: rename enable_redshift_push to enable_db_push since not redshift here.
        ...
        elapsed = end_time - start_time
        logger.info(
            "Process time to complete job (post db copies if any): {} s".format(elapsed)
        )
    ...
    def etl_no_io(self, sc, sc_sql, loaded_inputs={}, jargs=None):
        ...
        if (
            output is not None
            and self.jargs.output["type"] in self.TABULAR_TYPES
            and self.jargs.output.get("df_type", "spark") == "spark"
        ):
    ...
    def etl_no_io_function(
        self, sc, sc_sql, transform=None, transform_args={}, loaded_inputs={}
    ):
    ...
    def set_jargs(self, pre_jargs, loaded_inputs={}):
        ...
        )  # set yml_args=None so loading yml is handled in Job_Args_Parser()
    ...
    def load_inputs(self, loaded_inputs):
        ...
        for item in self.jargs.inputs.keys():
            ...
            if item in loaded_inputs.keys():
                ...
                logger.info(
                    "Input '{}' passed in memory from a previous job.".format(item)
                )
                continue
            ...
            if self.jargs.inputs[item]["type"] == "other":
                app_args[item] = None
                logger.info(
                    "Input '{}' not loaded since type set to 'other'.".format(item)
                )
                continue
        ...
        if self.jargs.is_incremental and self.jargs.inputs[item]["type"] not in (
            "mysql",
            "clickouse",
        ):
    ...
    def filter_incremental_inputs_motm(self, app_args):
        ...
        min_dt = (
            self.get_previous_output_max_timestamp(self.sc, self.sc_sql)
            if len(app_args.keys()) > 0
            else None
        )
        ...
        for item in app_args.keys():
            input_is_tabular = (
                self.jargs.inputs[item]["type"] in self.TABULAR_TYPES
                and self.jargs.inputs[item]("df_type", "spark") == "spark"
            )
        ...
        for item in app_args.keys():
            ...
            if inc:
                if input_is_tabular:
                    ...
                    logger.info(
                        "Input dataset '{}' will be filtered for min_dt={} max_dt={}".format(
                            item, min_dt, max_dt
                        )
                    )
                    if min_dt:
                        # min_dt = to_date(lit(s)).cast(TimestampType()  # TODO: deal with dt type, as coming from parquet
                        app_args[item] = app_args[item].filter(
                            app_args[item][inc] > min_dt
                        )
                    if max_dt:
                        app_args[item] = app_args[item].filter(
                            app_args[item][inc] <= max_dt
                        )
                else:
                    raise Exception(
                        "Incremental loading is not supported for unstructured input. You need to handle the incremental logic in the job code."
                    )
        return app_args
        ...
        for item in app_args.keys():
            ...
            if inc:
                if input_is_tabular:
                    ...
                    inc_type = {k: v for k, v in app_args[item].dtypes}[inc]
                    logger.info(
                        "Input dataset '{}' will be filtered for {}='{}'".format(
                            item, inc, self.period
                        )
                    )
                    app_args[item] = app_args[item].filter(
                        app_args[item][inc] == self.period
                    )
                else:
                    raise Exception(
                        "Incremental loading is not supported for unstructured input. You need to handle the incremental logic in the job code."
    ...
    def sql_register(self, app_args):
        for item in app_args.keys():
            input_is_tabular = hasattr(
                app_args[item], "rdd"
            )  # assuming DataFrame will keep 'rdd' attribute
    ...
    def load_input(self, input_name):
        ...
        if input_type in self.FILE_TYPES:
            ...
            path = (
                path.replace("s3://", "s3a://")
                if self.jargs.mode == "dev_local"
                else path
            )
            logger.info(
                "Input '{}' to be loaded from files '{}'.".format(input_name, path)
            )
        ...
        if self.jargs.inputs[input_name].get("df_type") == "pandas":
            if input_type == "csv":
                ...
                raise Exception(
                    "Unsupported input type '{}' for path '{}'. Supported types for pandas are: {}. ".format(
                        input_type,
                        self.jargs.inputs[input_name].get("path"),
                        self.PANDAS_DF_TYPES,
                    )
                )
            logger.info("Input '{}' loaded from files '{}'.".format(input_name, path))
        ...
        if input_type == "csv":
            ...
            raise Exception(
                "Unsupported input type '{}' for path '{}'. Supported types are: {}. ".format(
                    input_type,
                    self.jargs.inputs[input_name].get("path"),
        ...
        logger.info(
            "Input data types: {}".format(
                pformat([(fd.name, fd.dataType) for fd in sdf.schema.fields])
            )
        )
        return sdf
    ...
    def load_data_from_files(self, name, path, type, sc, sc_sql):
        ...
        input_name = name
        path = (
            path.replace("s3://", "s3a://") if self.jargs.mode == "dev_local" else path
        )
        logger.info(
            "Dataset '{}' to be loaded from files '{}'.".format(input_name, path)
        )
        ...
        if input_type == "csv":
            sdf = sc_sql.read.csv(
                path, header=True
            )  # TODO: add way to add .option("delimiter", ';'), useful for metric_budgeting.
            logger.info("Dataset '{}' loaded from files '{}'.".format(input_name, path))
        elif input_type == "parquet":
            ...
            raise Exception(
                "Unsupported dataset type '{}' for path '{}'. Supported types are: {}. ".format(
                    input_type, path, self.SUPPORTED_TYPES
                )
            )
    ...
    def load_mysql(self, input_name):
        ...
        url = "jdbc:mysql://{host}:{port}/{service}{extra_params}".format(
            host=db["host"],
            port=db["port"],
            service=db["service"],
        ...
        if not inc_field:
            ...
            higher_limit = (
                "AND {inc_field} < '{period_next}'".format(
                    inc_field=inc_field, period_next=self.period_next
                )
                if self.period_next
                else ""
            )
            query_str = "select * from {dbtable} where {inc_field} >= '{period}' {higher_limit}".format(
                dbtable=dbtable,
                inc_field=inc_field,
                period=self.period,
            ...
            logger.info(
                'Pulling table from mysql with query_str "{}"'.format(query_str)
            )
            ...
            sdf = (
                ...
                .option("password", db["password"])
                .option(
                    "customSchema", self.jargs.merged_args.get("jdbc_custom_schema", "")
                )
    ...
    def load_clickhouse(self, input_name):
        ...
        creds_section = self.jargs.inputs[input_name]["creds"]
        db = creds[creds_section]
        url = "jdbc:postgresql://{host}/{service}".format(
            host=db["host"], service=db["service"]
        )
        dbtable = self.jargs.inputs[input_name]["db_table"]
        inc_field = self.jargs.inputs[input_name].get("inc_field")
        if not inc_field:
            ...
            query_str = "select * from {} where {} = '{}'".format(
                dbtable, inc_field, period
            )
            logger.info(
                'Pulling table from Clickhouse with query_str "{}"'.format(query_str)
            )
            sdf = (
                self.sc_sql.read.format("jdbc")
                .option("driver", "org.postgresql.Driver")
    ...
    def get_previous_output_max_timestamp(self, sc, sc_sql):
        path = self.jargs.output[
            "path"
        ]  # implies output path is incremental (no "{now}" in string.)
        path += (
            "*" if self.jargs.merged_args.get("incremental_type") == "no_schema" else ""
        )  # '*' to go into output subfolders.
        try:
            df = self.load_data_from_files(
                name="output",
                path=path,
            ...
            logger.info(
                "Previous increment could not be loaded or doesn't exist. It will be ignored. Folder '{}' failed loading with error '{}'.".format(
                    path, e
                )
            )
            return None
    ...
    def save_output(self, output, now_dt=None):
        self.path = self.save(
            ...
            incremental_type=self.jargs.merged_args.get(
                "incremental_type", "no_schema"
            ),
            partitionby=self.jargs.output.get("inc_field")
            or self.jargs.merged_args.get("partitionby"),
            file_tag=self.jargs.merged_args.get("file_tag"),
        )  # TODO: make param standard in cmd_args ?
    ...
    def save(
        ...
        if is_incremental and incremental_type == "no_schema":
            current_time = now_dt.strftime(
                "%Y%m%d_%H%M%S_utc"
            )  # no use of now_dt to make it updated for each inc.
            file_tag = (
                ("_" + file_tag) if file_tag else ""
            )  # TODO: make that param standard in cmd_args ?
            path += "inc_{}{}/".format(current_time, file_tag)

        # TODO: rename 'partitioned' to 'spark_partitions' and 'no_schema' to 'yaetos_partitions'
        write_mode = (
            "append" if incremental_type == "partitioned" or partitionby else "error"
        )
        ...
        if self.jargs.output.get("df_type") == "pandas":
            if type == "csv":
                ...
                raise Exception(
                    "Need to specify supported output type for pandas, csv only for now."
                )
        ...
        if type == "txt":
            ...
            output.write.partitionBy(*partitionby).mode(write_mode).option(
                "header", "true"
            ).csv(path)
        else:
            raise Exception(
                "Need to specify supported output type, either txt, parquet or csv."
            )
    ...
    def copy_to_redshift_using_pandas(self, output, types):
        ...
        schema = (
            schema.format(schema=self.jargs.schema) if "{schema}" in schema else schema
        )
        creds = Cred_Ops_Dispatcher().retrieve_secrets(
            self.jargs.storage, creds=self.jargs.connection_file
        )
    ...
    def copy_to_redshift_using_spark(self, sdf):
        ...
        create_table(
            ...
            self.jargs.merged_args.get("spark_version", "2.4"),
        )
    ...
    def copy_to_clickhouse(self, sdf):
        ...
        create_table(
            sdf, connection_profile, name_tb, schema, creds, self.jargs.is_incremental
        )
    ...
    def send_msg(self, msg, recipients=None):
        ...
        if not recipients:
            logger.error(
                "Email can't be sent since no recipient set in {}, .\nMessage : \n{}".format(
                    self.jargs.job_param_file, msg
                )
            )
            return None
    ...
    def check_pk(df, pks):
        ...
        if count != count_pk:
            logger.error(
                "Given fields ({}) are not PKs since not unique. count={}, count_pk={}".format(
                    pks, count, count_pk
                )
            )
            return False
            ...
            logger.info(
                "Given fields ({}) are PKs (i.e. unique). count=count_pk={}".format(
                    pks, count
                )
            )
            return True
...
class Period_Builder:
    ...
    def get_last_output_to_last_day(
        self, last_run_period, first_day_input, as_of_date=datetime.utcnow()
    ):
    ...
    def generate_schemas(self, loaded_datasets, output):
        ...
        for key, value in loaded_datasets.items():
            if value:
                ...
                yml["inputs"][key] = {
                    fd.name: fd.dataType.__str__() for fd in value.schema.fields
                }
...
class Job_Yml_Parser:
    ...
    def __init__(self, job_name, job_param_file, mode, skip_job=False):
        ...
        self.yml_args["py_job"] = self.yml_args.get(
            "py_job"
        ) or self.set_py_job_from_name(job_name)
    ...
    def set_job_name_from_file(job_file):
        ...
        if job_file.startswith(CLUSTER_APP_FOLDER + "jobs/"):
        ...
        elif job_file.startswith(
            LOCAL_JOB_REPO_FOLDER + "jobs/"
        ):  # when run from external repo.
            ...
            job_name = job_file[
                job_file.find("/scripts.zip/jobs/") + len("/scripts.zip/jobs/") :
            ]
        else:
    ...
    def set_sql_file_from_name(job_name, mode):
        ...
        if mode in ("dev_EMR", "prod_EMR"):
            ...
            raise Exception(
                "Mode not supported in set_sql_file_from_name(): {}".format(mode)
            )
    ...
    def set_job_yml(self, job_name, job_param_file, yml_mode, skip_job):
        ...
        if job_name not in yml["jobs"] and not skip_job:
            raise KeyError(
                "Your job '{}' can't be found in jobs_metadata file '{}'. Add it there or make sure the name matches".format(
        ...
        if yml_mode not in yml["common_params"]["mode_specific_params"]:
            raise KeyError(
                "Your yml mode '{}' can't be found in jobs_metadata file '{}'. Add it there or make sure the name matches".format(
                    yml_mode, job_param_file
                )
            )
...
class Job_Args_Parser:
    ...
    def __init__(
        ...
        if yml_args is None:
            ...
            args["mode"] = (
                "dev_EMR"
                if args["mode"] == "dev_local"
                and args["deploy"] in ("EMR", "EMR_Scheduled")
                else args["mode"]
            )
            assert "job_name" in args.keys()
            yml_args = Job_Yml_Parser(
                args["job_name"],
                args["job_param_file"],
                args["mode"],
                args.get("skip_job", False),
            ).yml_args
    ...
    def get_deploy_args(self):
        return {
            key: value
            for key, value in self.merged_args.items()
            if key in self.DEPLOY_ARGS_LIST
        }
    ...
    def get_app_args(self):
        return {
            key: value
            for key, value in self.merged_args.items()
            if key not in self.DEPLOY_ARGS_LIST or key == "mode"
        }
    ...
    def update_args(self, args, loaded_inputs):
        ...
        args["is_incremental"] = self.set_is_incremental(
            args.get("inputs", {}), args.get("output", {})
        )
    ...
    def set_inputs(self, args, loaded_inputs):
        ...
        if loaded_inputs:
            return {
                key: {"path": val, "type": "df"} for key, val in loaded_inputs.items()
            }
        else:
    ...
    def set_is_incremental(self, inputs, output):
        return (
            any(["inc_field" in inputs[item] for item in inputs.keys()])
            or "inc_field" in output
        )
...
class Commandliner:
    def __init__(self, Job, **job_args):
        ...
        if (
            Job is None
        ):  # when job run from "python launcher.py --job_name=some_name_from_job_metadata_file"
            ...
            )  # can provide jargs directly here since job_file (and so job_name) needs to be extracted from job first. So, letting job build jargs.

        # Executing or deploying
        if job.jargs.deploy in ("none"):  # when executing job code
            self.launch_run_mode(job)
        elif job.jargs.deploy in (
            "EMR",
            "EMR_Scheduled",
            "code",
        ):  # when deploying to AWS for execution there
            self.launch_deploy_mode(
                job.jargs.get_deploy_args(), job.jargs.get_app_args()
            )
    ...
    def set_commandline_args(parser):
        ...
        cmd_unknown_args = dict(
            [item[2:].split("=") for item in cmd_unknown_args]
        )  # imposes for unknown args to be defined with '=' and to start with '--'
    ...
    def define_commandline_args():
        ...
        parser.add_argument(
            "-d",
            "--deploy",
            choices=set(["none", "EMR", "EMR_Scheduled", "EMR_DataPipeTest", "code"]),
            help="Choose where to run the job.",
        )
        parser.add_argument(
            "-m",
            "--mode",
            choices=set(["dev_local", "dev_EMR", "prod_EMR"]),
            help="Choose which set of params to use from jobs_metadata.yml file.",
        )
        parser.add_argument(
            "-j",
            "--job_param_file",
            help="Identify file to use. It can be set to 'False' to not load any file and provide all parameters through job or command line arguments.",
        )
        ...
        parser.add_argument(
            "--connection_file", help="Identify file to use. Default to repo one."
        )
        parser.add_argument(
            "--jobs_folder",
            help="Identify the folder where job code is. Necessary if job code is outside the repo, i.e. if this is used as an external library. By default, uses the repo 'jobs/' folder.",
        )
        parser.add_argument(
            "-s",
            "--storage",
            choices=set(["local", "s3"]),
            help="Choose 'local' (default) or 's3'.",
        )
        parser.add_argument(
            "-x",
            "--dependencies",
            action="store_true",
            help="Run the job dependencies and then the job itself",
        )
        parser.add_argument(
            "-c",
            "--rerun_criteria",
            choices=set(["last_date", "output_empty", "both"]),
            help="Choose criteria to rerun the next increment or not. 'last_date' usefull if we know data goes to a certain date. 'output_empty' not to be used if increment may be empty but later ones not. Only relevant for incremental job.",
        )
        parser.add_argument(
            "--chain_dependencies",
            action="store_true",
            help="Run dependant jobs in a chained way, i.e. passing output to next step without dropping to disk. Only useful if ran with dependencies (-x) and requires output to be dataframes.",
        )
        parser.add_argument(
            "-l",
            "--load_connectors",
            choices=set(["all", "none"]),
            help="Load java packages to enable spark connectors (s3, redshift, mysql). Set to 'none' to have faster spark start time and smaller log when connectors are not necessary. Only useful when mode=dev_local.",
        )
        parser.add_argument(
            "-t",
            "--output.type",
            choices=set(["csv", "parquet"]),
            help="Override output type. Useful for development. Can be ignored otherwise.",
        )
        # Deploy specific
        parser.add_argument(
            "--aws_config_file", help="Identify file to use. Default to repo one."
        )
        parser.add_argument(
            "-a",
            "--aws_setup",
            help="Choose aws setup from conf/aws_config.cfg, typically 'prod' or 'dev'. Only relevant if choosing to deploy to a cluster.",
        )
        parser.add_argument(
            "-o",
            "--leave_on",
            action="store_true",
            help="Use arg to not terminate cluster after running the job. Mostly for testing. Only relevant when creating a new cluster when deploy=EMR.",
        )
        parser.add_argument(
            "-p",
            "--push_secrets",
            action="store_true",
            help="Pushing secrets to cluster. Only relevant if choosing to deploy to a cluster.",
        )
    ...
    def launch_run_mode(self, job):
        ...
        if job.jargs.spark_boot is True:
            sc, sc_sql = self.create_contexts(
                app_name, job.jargs
            )  # TODO: set spark_version default upstream, remove it from here and from deploy.py.
        else:
            sc, sc_sql = None, None
    ...
    def create_contexts(self, app_name, jargs):
        ...
        if jargs.merged_args.get(
            "driver-memoryOverhead"
        ):  # For extra overhead for python in driver (typically pandas)
            conf = conf.set(
                "spark.driver.memoryOverhead",
                jargs.merged_args["driver-memoryOverhead"],
            )
        ...
        if jargs.mode == "dev_local" and jargs.load_connectors == "all":
            ...
            package = (
                PACKAGES_LOCAL
                if jargs.merged_args.get("spark_version", "2.4") == "2.4"
                else PACKAGES_LOCAL_ALT
            )
            package_str = ",".join(package)
            conf = conf.set("spark.jars.packages", package_str).set("spark.jars", JARS)
        ...
        if jargs.merged_args.get("emr_core_instances") == 0:
            conf = conf.set("spark.hadoop.fs.s3a.buffer.dir", "/tmp").set(
...
class Flow:
    def __init__(self, launch_jargs, app_name):
        ...
        df = self.create_connections_jobs(
        ...
        logger.info("-" * 80)
        logger.info("-")
        launch_jargs.cmd_args.pop(
            "job_name", None
        )  # removing since it should be pulled from yml and not be overriden by cmd_args.
    ...
    def run_pipeline(self, sc, sc_sql):
        ...
        for job_name in self.leafs:
            ...
            # Get yml
            yml_args = Job_Yml_Parser(
                job_name, self.launch_jargs.job_param_file, self.launch_jargs.mode
            ).yml_args
            # Get loaded_inputs
            loaded_inputs = {}
            if self.launch_jargs.chain_dependencies:
                if yml_args.get("inputs", "no input") == "no input":
                    raise Exception(
                        "Pb with loading job_yml or finding 'inputs' parameter in it, so 'chain_dependencies' argument not useable in this case."
                    )
            ...
            # Get jargs
            jargs = Job_Args_Parser(
                self.launch_jargs.defaults_args,
                yml_args,
                self.launch_jargs.job_args,
                self.launch_jargs.cmd_args,
                loaded_inputs=loaded_inputs,
            )
            ...
            df[job_name] = job.etl(
                sc, sc_sql
            )  # at this point df[job_name] is unpersisted. TODO: keep it persisted.

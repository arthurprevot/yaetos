...
class Job_Args_Parser():
    ...
    def get_mode(args):
        ...
        if args.get('deploy') in ('EMR', 'EMR_Serverless', 'k8s', 'EMR_Scheduled', 'airflow', 'airflow_k8s') and args.get('mode') and 'dev_local' in args['mode'].split(','):  # using 'dev_local' because set by default.
...
class Runner():
    ...
    def run(self):
        ...
        elif jargs.deploy in ('EMR', 'EMR_Serverless', 'k8s', 'EMR_Scheduled', 'airflow', 'airflow_k8s', 'code'):  # when deploying to AWS for execution there
    ...
    def define_commandline_args():
        ...
        parser.add_argument("-d", "--deploy", choices=set(['none', 'EMR', 'EMR_Serverless', 'k8s', 'EMR_Scheduled', 'airflow', 'airflow_k8s', 'EMR_DataPipeTest', 'code', 'local_spark_submit']), help="Choose where to run the job.")

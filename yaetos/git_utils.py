...
class GitConfigManager:
    ...
    def get_config_from_git(self, local_app_folder):
        ...
        branch_yaetos = (
            subprocess.check_output(["git", "describe", "--all"], cwd=local_app_folder)
            .strip()
            .decode("ascii")
        )
        last_commit_yaetos = (
            subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=local_app_folder)
            .strip()
            .decode("ascii")
        )
        diffs_yaetos = (
            subprocess.check_output(["git", "diff", "HEAD"], cwd=local_app_folder)
            .strip()
            .decode("ascii")
        )
    ...
    def is_git_controlled(self):
        out = os.system(
            "git rev-parse"
        )  # not using subprocess.check_output() to avoid crash if it fails.
        if out == 0:
            return True
        else:

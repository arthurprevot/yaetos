...
class FSOpsDispatcher:
    """Set of functions to dispatch mostly IO methods to local or cloud depending on the path being local or cloud (s3://*)."""

    @staticmethod
    def is_s3_path(path: str) -> bool:
        """
        Returns own definition if a string is an s3 path or not.
        """
        return bool(re.search(r"s3a?://", path))

    @staticmethod
    def split_s3_path(s3_path: str) -> Tuple[str, str, List[str]]:
        """
        Splits a path to an s3 path in bucket name, bucket filename and filename parts.
        """
        bucket_pattern = r"s3a?://(?P<bucket_name>(.+?)(?=/))(?P<file_key>.*)"
        extracted_groups = re.search(bucket_pattern, s3_path)
        if not extracted_groups or len(extracted_groups.groups()) < 2:
            raise RuntimeError(
                f"Could not parse all the parts for the s3 path {s3_path}"
            )
        bucket_name = extracted_groups.group("bucket_name")
        file_key = extracted_groups.group("file_key")
        return (bucket_name, file_key, [f for f in file_key.split("/") if f != ""])

    # --- save_metadata set of functions ----
    def save_metadata(self, fname, content):
        self.save_metadata_cluster(fname, content) if self.is_s3_path(
            fname
    ...
    def save_file(self, fname, content):
        self.save_file_cluster(fname, content) if self.is_s3_path(
            fname
        ) else self.save_file_local(fname, content)
    ...
    def save_file_cluster(self, fname, content):
        ...
        logger.info(
            "Pushed local file to S3, from '{}' to '{}' ".format(local_path, fname)
        )

    # --- load_file set of functions ----
    def load_file(self, fname):
        return (
            self.load_file_cluster(fname)
            if self.is_s3_path(fname)
            else self.load_file_local(fname)
        )
    ...
    def listdir(self, path):
        return (
            self.listdir_cluster(path)
            if self.is_s3_path(path)
            else self.listdir_local(path)
        )
    ...
    def listdir_cluster(
        path,
    ):  # TODO: rename to listdir_s3, same for similar functions from FSOpsDispatcher
        ...
        if path.startswith("s3://"):
            ...
            s3_root = (
                "s3a://"  # necessary when pulling S3 to local automatically from spark.
            )
        else:
            raise ValueError(
                'Problem with path. Pulling from s3, it should start with "s3://" or "s3a://". Path is: {}'.format(
                    path
                )
            )
        ...
        paths = [
            item["Prefix"].split("/")[-2] for item in objects.search("CommonPrefixes")
        ]
        return paths
    ...
    def dir_exist(self, path):
        return (
            ...
            if self.is_s3_path(path)
            else self.dir_exist_local(path)
        )
    ...
    def load_pandas(self, fname, file_type, read_func, read_kwargs):
        return (
            self.load_pandas_cluster(fname, file_type, read_func, read_kwargs)
            if self.is_s3_path(fname)
            else self.load_pandas_local(fname, file_type, read_func, read_kwargs)
        )
    ...
    def load_pandas_cluster(self, fname, file_type, read_func, read_kwargs):
        ...
        cp = CloudPath(
            fname
        )  # TODO: add way to load it with specific profile_name or client, as in "s3c = boto3.Session(profile_name='default').client('s3')"
        logger.info(
            "Copying files from S3 '{}' to local '{}'. May take some time.".format(
                fname, local_path
            )
        )
    ...
    def save_pandas(self, df, fname, save_method, save_kwargs):
        return (
            self.save_pandas_cluster(df, fname, save_method, save_kwargs)
            if self.is_s3_path(fname)
            else self.save_pandas_local(df, fname, save_method, save_kwargs)
        )
    ...
    def save_pandas_cluster(self, df, fname, save_method, save_kwargs):
        ...
        with StringIO() as file_buffer:
            ...
            response = s3c.put_object(
                Bucket=bucket_name, Key=bucket_fname, Body=file_buffer.getvalue()
            )
...
class Cred_Ops_Dispatcher:
    def retrieve_secrets(self, storage, creds="conf/connections.cfg"):
        creds = (
            self.retrieve_secrets_cluster()
            if storage == "s3"
            else self.retrieve_secrets_local(creds)
        )
        return creds

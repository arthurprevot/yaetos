...
class DeployPySparkScriptOnAws:
    ...
    def __init__(self, deploy_args, app_args):
        ...
        self.app_file = app_args[
            "py_job"
        ]  # TODO: remove all refs to app_file to be consistent.
        ...
        self.emr_core_instances = int(
            app_args.get("emr_core_instances", 1)
        )  # TODO: make this update EMR_Scheduled mode too.
        self.deploy_args = deploy_args
        self.ec2_instance_master = app_args.get(
            "ec2_instance_master", "m5.xlarge"
        )  #'m5.12xlarge', # used m3.2xlarge (8 vCPU, 30 Gib RAM), and earlier m3.xlarge (4 vCPU, 15 Gib RAM)
        ...
        self.pipeline_name = self.generate_pipeline_name(
            self.deploy_args["mode"], self.app_args["job_name"], self.user
        )  # format: some_job.some_user.20181204.153429
        self.job_log_path = (
            self.get_job_log_path()
        )  # format: yaetos/logs/some_job.some_user.20181204.153429
        self.job_log_path_with_bucket = "{}/{}".format(
            self.s3_bucket_logs, self.job_log_path
        )  # format: bucket-tempo/yaetos/logs/some_job.some_user.20181204.153429
        self.package_path = (
            self.job_log_path + "/code_package"
        )  # format: yaetos/logs/some_job.some_user.20181204.153429/package
        self.package_path_with_bucket = (
            self.job_log_path_with_bucket + "/code_package"
        )  # format: bucket-tempo/yaetos/logs/some_job.some_user.20181204.153429/package
        ...
        try:
            self.git_yml = GitConfigManager().get_config_from_git(eu.LOCAL_APP_FOLDER)
            GitConfigManager().save_yaml(self.git_yml)
        except Exception as e:  # TODO: get specific exception
            self.git_yml = None
            logger.info(
                "Error saving yml file with git info, with error '{}'.".format(e)
            )
    ...
    def continue_post_git_check(self):
        ...
        git_yml = {
            key: value
            for key, value in self.git_yml.items()
            if key
            in (
                "is_dirty_yaetos",
                "is_dirty_current",
        ...
        if self.git_yml["is_dirty_current"] or self.git_yml["is_dirty_yaetos"]:
            print(
                "Some changes to your git controled files are not committed to git: {}".format(
                    git_yml
                )
            )
            ...
            if answer == "y":
                ...
                print(
                    'Answer not understood, it should be "y" or "n", cancelling deployment'
                )
                return False
        else:
            print(
                "Git controled files are clean, continuing with push to prod. Git setup: {}".format(
                    git_yml
                )
            )
            return True
    ...
    def run_push_code(self):
        ...
        if self.deploy_args.get("push_secrets", False):
            self.push_secrets(
                creds_or_file=self.app_args["connection_file"]
    ...
    def run_direct(self):
        ...
        if new_cluster:
            ...
            print(
                "cluster name: %s, and id: %s" % (self.pipeline_name, self.cluster_id)
            )
            self.step_run_setup_scripts(c)
        else:
            print(
                "Reusing existing cluster, name: %s, and id: %s"
        ...
        if (
            new_cluster
            and not self.deploy_args.get("leave_on")
            and self.app_args.get("clean_post_run")
        ):  # TODO: add clean_post_run in input options.
            logger.info("New cluster setup to be deleted after job finishes.")
            self.describe_status_until_terminated(c)
            self.remove_temp_files(
                s3
            )  # TODO: remove tmp files for existing clusters too but only tmp files for the job
    ...
    def get_active_clusters(self, c):
        ...
        clusters = [
            (ii + 1, item["Id"], item["Name"])
            for ii, item in enumerate(response["Clusters"])
        ]
        return clusters
    ...
    def choose_cluster(self, clusters, cluster_id=None):
        ...
        if cluster_id is not None:
            ...
            return {"id": cluster_id, "name": None}
    ...
    def get_job_name(pipeline_name):
        ...
        return (
            pipeline_name.split("__")[2].replace("_d_", ".").replace("_s_", "/")
            if "__" in pipeline_name
            else None
        )
    ...
    def temp_bucket_exists(self, s3):
        ...
        try:
            ...
            if error_code == 404:
                terminate(
                    "Bucket for temporary files does not exist: "
                    + self.s3_bucket_logs
                    + " "
                    + e.message
                )
            terminate(
                "Error while connecting to temporary Bucket: "
                + self.s3_bucket_logs
                + " "
                + e.message
            )
    ...
    def tar_python_scripts(self):
        ...
        for f in files:
            t_file.add(
                base + "yaetos/" + f,
                arcname="yaetos/" + f,
                filter=lambda obj: obj if obj.name.endswith(".py") else None,
            )
        ...
        for f in files:
            t_file.add(
                base + "yaetos/libs/analysis_toolkit/" + f,
                arcname="yaetos/libs/analysis_toolkit/" + f,
                filter=lambda obj: obj if obj.name.endswith(".py") else None,
            )
        ...
        for f in files:
            t_file.add(
                base + "yaetos/libs/python_db_connectors/" + f,
                arcname="yaetos/libs/python_db_connectors/" + f,
                filter=lambda obj: obj if obj.name.endswith(".py") else None,
            )
    ...
    def get_package_path(self):
        ...
        if self.app_args["code_source"] == "lib":
            ...
            if len(bases) > 1:
                logger.info(
                    "There is more than one source of code to ship to EMR '{}'. Will continue with the first one.".format(
                        bases
                    )
                )
            base = bases[0] + "/"
    ...
    def upload_temp_files(self, s3):
        ...
        setup_master = (
            "setup_master.sh"
            if self.deploy_args.get("spark_version", "2.4") == "2.4"
            else "/setup_master_alt.sh"
        )
        setup_nodes = (
            "setup_nodes.sh"
            if self.deploy_args.get("spark_version", "2.4") == "2.4"
            else "/setup_nodes_alt.sh"
        )

        # Looping through all 4 steps below doesn't work (Fails silently) so done 1 by 1.
        s3.Object(self.s3_bucket_logs, self.package_path + "/setup_master.sh").put(
            Body=open(self.TMP + setup_master, "rb"), ContentType="text/x-sh"
        )
        s3.Object(self.s3_bucket_logs, self.package_path + "/setup_nodes.sh").put(
            Body=open(self.TMP + setup_nodes, "rb"), ContentType="text/x-sh"
        )
        s3.Object(
            self.s3_bucket_logs, self.package_path + "/terminate_idle_cluster.sh"
        ).put(
            Body=open(self.TMP + "terminate_idle_cluster.sh", "rb"),
            ContentType="text/x-sh",
        )
        s3.Object(self.s3_bucket_logs, self.package_path + "/scripts.tar.gz").put(
            Body=open(self.TMP + "scripts.tar.gz", "rb"),
            ContentType="application/x-tar",
        )
        logger.info(
            "Uploaded job files (scripts.tar.gz, {}, {}, terminate_idle_cluster.sh) to bucket path '{}/{}'".format(
                setup_master, setup_nodes, self.s3_bucket_logs, self.package_path
            )
        )
        return True
    ...
    def remove_temp_files(self, s3):
        ...
        for key in bucket.objects.all():
            if key.key.startswith(self.pipeline_name) is True:
                key.delete()
                logger.info(
                    "Removed '{}' from bucket for temporary files".format(key.key)
                )
    ...
    def start_spark_cluster(self, c, emr_version):
        ...
        response = c.run_job_flow(
            ...
            LogUri="s3://{}/{}/manual_run_logs/".format(
                self.s3_bucket_logs, self.metadata_folder
            ),
            ...
            BootstrapActions=[
                {
                    ...
                    "ScriptBootstrapAction": {
                        "Path": "s3n://{}/setup_nodes.sh".format(
                            self.package_path_with_bucket
                        ),
                        "Args": [],
                    },
        ...
        logger.info(
            "Created Spark EMR cluster ({}) with cluster_id {}".format(
                emr_version, self.cluster_id
            )
        )
    ...
    def step_spark_submit(self, c, app_file, app_args):
        ...
        logger.info(
            "Added step 'spark-submit' with command line '{}'".format(cmd_runner_args)
        )
        time.sleep(1)  # Prevent ThrottlingException
    ...
    def get_spark_submit_args(self, app_file, app_args):
        ...
        med = (
            ["--driver-memory={}".format(app_args["driver-memory"])]
            if app_args.get("driver-memory")
            else []
        )
        cod = (
            ["--driver-cores={}".format(app_args["driver-cores"])]
            if app_args.get("driver-cores")
            else []
        )
        mee = (
            ["--executor-memory={}".format(app_args["executor-memory"])]
            if app_args.get("executor-memory")
            else []
        )
        coe = (
            ["--executor-cores={}".format(app_args["executor-cores"])]
            if app_args.get("executor-cores")
            else []
        )
        ...
        jop = (
            [
                "--job_param_file={}".format(
                    eu.CLUSTER_APP_FOLDER + eu.JOBS_METADATA_FILE
                )
            ]
        ...
        sql = (
            []
        )  # ["--sql_file={}".format(eu.CLUSTER_APP_FOLDER+app_args['sql_file'])] if app_args.get('sql_file') else [] # not needed when sql job is run from jobs/generic/launcher.py. TODO: make it work when launching from jobs/generic/sql_job.py
        nam = (
            ["--job_name={}".format(app_args["job_name"])]
    ...
    def run_aws_data_pipeline(self):
        ...
        parameterValues = self.define_data_pipeline(
            client, pipe_id, self.emr_core_instances
        )
    ...
    def create_data_pipeline(self, client):
        ...
        create = client.create_pipeline(
            name=self.pipeline_name, uniqueId=str(unique_id)
        )
        ...
        logger.debug(
            ...
            + str(client.describe_pipelines(pipelineIds=[pipe_id]))
        )
        return pipe_id
    ...
    def define_data_pipeline(self, client, pipe_id, emr_core_instances):
        ...
        if emr_core_instances != 0:
            definition_file = (
                base + "yaetos/definition.json"
            )  # see syntax in datapipeline-dg.pdf p285 # to add in there: /*"AdditionalMasterSecurityGroups": "#{}",  /* To add later to match EMR mode */
        else:
        ...
        definition = json.load(
            open(definition_file, "r")
        )  # Note: Data Pipeline doesn't support emr-6.0.0 yet.
        ...
        for item in pipelines:
            ...
            if job_name == self.app_args["job_name"]:
                response = client.deactivate_pipeline(
                    pipelineId=item["id"], cancelActive=True
                )
                logger.info(
                    "Deactivated pipeline {}, {}, {}".format(
                        job_name, item["name"], item["id"]
                    )
                )
    ...
    def update_params(self, parameterValues):
        ...
        # Change key pair
        myScheduleType = {"EMR_Scheduled": "cron", "EMR_DataPipeTest": "ONDEMAND"}[
            self.deploy_args.get("deploy")
        ]
        myPeriod = self.deploy_args["frequency"] or "1 Day"
        if self.deploy_args["start_date"] and isinstance(
            self.deploy_args["start_date"], datetime
        ):
            myStartDateTime = self.deploy_args["start_date"].strftime(
                "%Y-%m-%dT%H:%M:%S"
            )
        elif self.deploy_args["start_date"] and isinstance(
            self.deploy_args["start_date"], str
        ):
            myStartDateTime = self.deploy_args["start_date"].format(
        ...
        for ii, item in enumerate(parameterValues):
            if "myEC2KeyPair" in item.values():
                parameterValues[ii] = {
                    "id": u"myEC2KeyPair",
                    "stringValue": self.ec2_key_name,
                }
            elif "mySubnet" in item.values():
                parameterValues[ii] = {
                    "id": u"mySubnet",
                    "stringValue": self.ec2_subnet_id,
                }
            elif "myPipelineLogUri" in item.values():
                parameterValues[ii] = {
                    "id": u"myPipelineLogUri",
                    "stringValue": "s3://{}/{}/scheduled_run_logs/".format(
                        self.s3_bucket_logs, self.metadata_folder
                    ),
                }
                ...
                parameterValues[ii] = {
                    "id": u"myScheduleType",
                    "stringValue": myScheduleType,
                }
            elif "myPeriod" in item.values():
                parameterValues[ii] = {"id": u"myPeriod", "stringValue": myPeriod}
            elif "myStartDateTime" in item.values():
                parameterValues[ii] = {
                    "id": u"myStartDateTime",
                    "stringValue": myStartDateTime,
                }
            elif "myBootstrapAction" in item.values():
                parameterValues[ii] = {
                    "id": u"myBootstrapAction",
                    "stringValue": bootstrap,
                }
            elif "myTerminateAfter" in item.values():
                parameterValues[ii] = {
                    "id": u"myTerminateAfter",
                    "stringValue": self.deploy_args.get(
                        "terminate_after", "180 Minutes"
                    ),
                }
            elif "myEMRReleaseLabel" in item.values():
                parameterValues[ii] = {
                    "id": u"myEMRReleaseLabel",
                    "stringValue": self.emr_version,
                }
            elif "myMasterInstanceType" in item.values():
                parameterValues[ii] = {
                    "id": u"myMasterInstanceType",
                    "stringValue": self.ec2_instance_master,
                }
            elif "myCoreInstanceCount" in item.values():
                parameterValues[ii] = {
                    "id": u"myCoreInstanceCount",
                    "stringValue": str(self.emr_core_instances),
                }
            elif "myCoreInstanceType" in item.values():
                parameterValues[ii] = {
                    "id": u"myCoreInstanceType",
                    "stringValue": self.ec2_instance_slaves,
                }

        # Change steps to include proper path
        setup_command = "s3://elasticmapreduce/libs/script-runner/script-runner.jar,s3://{s3_tmp_path}/setup_master.sh,s3://{s3_tmp_path}".format(
            s3_tmp_path=self.package_path_with_bucket
        )  # s3://elasticmapreduce/libs/script-runner/script-runner.jar,s3://bucket-tempo/ex1_frameworked_job.arthur_user1.20181129.231423/setup_master.sh,s3://bucket-tempo/ex1_frameworked_job.arthur_user1.20181129.231423/
        spark_submit_command = "command-runner.jar," + ",".join(
            [
                item.replace(",", "\\\,")
                for item in self.get_spark_submit_args(self.app_file, self.app_args)
            ]
        )  # command-runner.jar,spark-submit,--py-files,/home/hadoop/app/scripts.zip,--packages=com.amazonaws:aws-java-sdk-pom:1.11.760\\\\,org.apache.hadoop:hadoop-aws:2.7.0,/home/hadoop/app/jobs/examples/ex1_frameworked_job.py,--storage=s3  # instructions about \\\ part: https://docs.aws.amazon.com/datapipeline/latest/DeveloperGuide/dp-object-emractivity.html

        commands = [setup_command, spark_submit_command]
        mm = 0
        for ii, item in enumerate(parameterValues):
            if (
                "myEmrStep" in item.values() and mm < 2
            ):  # TODO: make more generic and cleaner
    ...
    def push_secrets(self, creds_or_file):
        ...
        try:
            ...
            logger.info(
                "Created aws secret, from {}, under secret_id:{}".format(
                    creds_or_file, eu.AWS_SECRET_ID
                )
            )
            ...
            logger.info(
                "Updated aws secret, from {}, under secret_id:{}".format(
                    creds_or_file, eu.AWS_SECRET_ID
                )
            )
    ...
    def get_yml(args):
        ...
        if meta_file is "repo":
            meta_file = (
                eu.CLUSTER_APP_FOLDER + eu.JOBS_METADATA_FILE
                if args["storage"] == "s3"
                else eu.JOBS_METADATA_LOCAL_FILE
            )
...
def deploy_standalone(job_args_update={}):
    ...
    if (
        jargs.deploy_option == "deploy_job"
    ):  # can be used to push random code to cluster
        ...
        deployer = DeployPySparkScriptOnAws(
            deploy_args, app_args
        )  # TODO: should remove need for some of these inputs as they are not required by tar_python_scripts()

...
sudo pip3 install boto3==1.9.57  # 1.18.41
...
sudo pip3 install pandas==1.0.0  # downgraded from 1.0.4 to be compatible with koalas. 0.25.1
sudo pip3 install sqlalchemy==1.3.15  # don't use 1.3.17 since not compatible with sqlalchemy-redshift (0.7.7). See if it is still a pb at https://github.com/sqlalchemy-redshift/sqlalchemy-redshift/issues/195

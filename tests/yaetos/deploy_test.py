...
class Test_DeployPySparkScriptOnAws(object):
    ...
    def test_temp_bucket_exists_success(self, deployer):
        mock_s3 = Mock()
    ...
    def test_temp_bucket_exists_failure(self, deployer):
        mock_s3 = Mock()
    ...
    def test_continue_post_git_check_no_prod(self, mock_isfile, deployer):
        deployer.app_args['mode'] = 'dev_EMR'
        assert deployer.continue_post_git_check() is True

    @patch('boto3.session.Session')
    def test_s3_ops(self, mock_session, deployer):
        mock_s3 = Mock()
    ...
    def test_tar_python_scripts(self, mock_walk, mock_listdir, mock_tarfile, deployer, tmp_path):
        # Setup mocks
    ...
    def test_run_direct(self, mock_run_direct, deployer):
        deployer.run_direct()
        mock_run_direct.assert_called_once()

    def test_get_active_clusters(self, deployer):
        mock_client = Mock()
    ...
    def test_choose_cluster(self, mock_input, deployer):
        clusters = [
    ...
    def test_start_spark_cluster(self, mock_start_cluster, deployer):
        mock_client = Mock()
    ...
    def test_run_aws_data_pipeline(self, mock_run_pipeline, deployer):
        deployer.run_aws_data_pipeline()
        mock_run_pipeline.assert_called_once()

    def test_create_data_pipeline(self, deployer):
        mock_client = Mock()
    ...
    def test_define_data_pipeline(self, deployer):
        mock_client = Mock()
        pipe_id = 'df-123'
        emr_instances = 2
    ...
    @pytest.mark.skip()  # TODO: skipped since AWS DP not used. Remove code all together later.
    def test_list_data_pipeline(self, deployer):
        mock_client = Mock()
    ...
    def test_run_aws_airflow(self, mock_run_airflow, deployer):
    ...
    def test_set_job_dag_name(self, deployer):
        job_name = 'test_job'
    ...
    def test_upload_dags(self, mock_upload, deployer):
        mock_s3 = Mock()
    ...
    def test_full_emr_deployment_flow(self, mock_submit, mock_start_cluster, mock_s3_ops, deployer):
        mock_s3 = Mock()
        mock_emr = Mock()
        mock_session = Mock()
        mock_session.resource.return_value = mock_s3
    ...
    def test_full_data_pipeline_flow(self, mock_activate, mock_define, mock_s3_ops, deployer):
        mock_pipeline = Mock()
        mock_session = Mock()
